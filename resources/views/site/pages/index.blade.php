...
@section('content')
        ...
        <div class="bn-news">
            <ul>
                @foreach ($top_news as $tp_news)
                    <li><a href="{{ route('newsIndex', ['news' => $tp_news]) }}">{{ Str::limit($tp_news->title, 100) }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
        ...
        <div id="slider" class="sl-slider-wrapper">
            <div class="sl-slider">
                @foreach ($banners as $key => $banner)
                    <div class="sl-slide" data-orientation="{{ $key % 2 == 0 ? 'horizontal' : 'vertical' }}"
                        data-slice1-rotation="-25" data-slice2-rotation="-25" data-slice1-scale="2" data-slice2-scale="2">
    ...
    <div class="item">
                                ...
                                <div class="news-text-wrapper">
                                    <h3 class="news-title"><span>{{ $s_new->category->name }}</span>{{ $s_new->title }}
                                    </h3>
                                        ...
                                        <div class="slider-match-details">
                                            <div class="the-tournament">
                                                <img src="{{ getimg($prev_match->Champion->image) }}"
                                                    data-toggle="tooltip" title="{{ $prev_match->Champion->title }}">
                                            </div>
                                            <div class="match-common-st">
                                                <div class="single-side team-l-wrapper">
                                                    <img src="{{ getimg($prev_match->FirstTeam->image) }}" alt="logo"
                                                        data-toggle="tooltip" title="{{ $prev_match->FirstTeam->name }}">
                                                </div>
                                                ...
                                                <div class="single-side team-l-wrapper">
                                                    <img src="{{ getimg($prev_match->SecondTeam->image) }}" alt="logo"
                                                        data-toggle="tooltip"
                                        ...
                                        <div class="slider-match-details">
                                            <div class="the-tournament">
                                                <img src="{{ getimg($next_match->Champion->image) }}"
                                                    data-toggle="tooltip" title="{{ $next_match->Champion->title }}">
                                            </div>
                                            <div class="match-common-st">
                                                <div class="single-side team-l-wrapper">
                                                    <img src="{{ getimg($next_match->FirstTeam->image) }}" alt="logo"
                                                ...
                                                <div class="single-side team-l-wrapper">
                                                    <img src="{{ getimg($next_match->SecondTeam->image) }}" alt="logo"
                                                        data-toggle="tooltip"
                            ...
                            <div class="bee3D--inner">
                                <div class="slider-match-timer-wrapper">
                                    <div class="slider-match-timer">
                                        ...
                                        <div class="slider-match-details">
                                            <div class="the-tournament">
                                                <img src="{{ getimg($nx_match->Champion->image) }}" data-toggle="tooltip"
                                                    title="{{ $nx_match->Champion->title }}">
                                            </div>
                                            <div class="match-common-st">
                                                <div class="single-side team-l-wrapper">
                                                    <img src="{{ getimg($nx_match->FirstTeam->image) }}" alt="logo"
                                                        data-toggle="tooltip" title="{{ $nx_match->FirstTeam->name }}">
                                                </div>
                                                <div class="single-side">
                                                    <div class="sm-size">@lang('trans.start')</div>
                                                    <div class="red-color sin-middle">
                                                ...
                                                <div class="single-side team-l-wrapper">
                                                    <img src="{{ getimg($nx_match->SecondTeam->image) }}" alt="logo"
                                                        data-toggle="tooltip" title="{{ $nx_match->SecondTeam->name }}">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="container">
        <div class="row">
            <div class="multimedia-slider owl-carousel owl-theme" id="multimedia-slider">
                @foreach (\App\News::latest()->get() as $media)
                    <div class="item">
                        <div class="news-image-wrapper">
                            <a class="watch-vid-btn" data-fancybox="multimedia" data-caption="{{ $media->title }}"
                                href="{{ getimg($media->image) }}" data-toggle="tooltip" title="شاهد المرفق">
                ...
                @foreach ($partners as $partner)
                    <div class="item">
                        <a href="{{ $partner->url }}" target="_blank" rel="noreferrer noopener">
                            <img src="{{ getimg($partner->image) }}" data-toggle="tooltip"
                                title="{{ $partner->name }}">
                        </a>
                    </div>
                @endforeach
...
@section('scripts')
    ...
    <script type="text/javascript">
        var aimTime =
            {{ is_null($next_match) ? date('Y-m-d H:i:s') : $next_match->date->diffInRealMilliseconds(now()->format('Y-m-d H:i:s')) }};
